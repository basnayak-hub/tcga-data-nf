...
print("hello!")
# this is temporary until we put a PR in for the MC pvalues
#sys.path.insert(0,'/Users/kas0607/research/repos/netZooPy/netZooPy')
...
meth = pandas.read_table(meth_file,header=0,index_col=0,sep=",")
print(meth.shape)
meth = meth.add_suffix('_methylation')
meth = meth.rename(index=str, columns={'TCGAbarcode_methylation':'TCGAbarcode'})

expr = pandas.read_table(expr_file,header=0,index_col=0,sep=",")
print(expr.shape)
...
all_data = pandas.merge(meth,expr,on="TCGAbarcode",how="inner")
...
df.to_csv(uuid + "_dragon_mat.tsv",sep="\t")
all_data.to_csv(uuid + "_dragon_input.tsv",sep="\t")
