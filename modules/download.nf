...
workflow downloadWf {
    ...
    if (expression_recount3 != [null]) {
        channelRecount = expression_recount3.map { item ->
            tuple(
                item.key,
                item.value.project,
    ...
    if (mutation_tcgabiolinks != [null]) {
        channelMutation = mutation_tcgabiolinks.map { item ->
            tuple(
                item.key,
                item.value.project,
    ...
    if (cnv_tcgabiolinks != [null]) {
        channelCNV = cnv_tcgabiolinks.map { item ->
            tuple(
                item.key,
                item.value.project,
    ...
    if (methylation_gdc != [null]) {
        channelMethylation = methylation_gdc.map { item ->
            tuple(
                item.key,
                item.value.project,
                item.value.gdc_type,
    ...
    if (clinical_tcgabiolinks != [null]) {
        channelClinical = clinical_tcgabiolinks.map { item ->
            tuple(
                item.key,
                item.value.project,
                item.value.data_category,
...
workflow fullDownloadWf {
    ...
    // DOWNLOAD RECOUNT3
    dChRe = dataCh.map { it ->
        if (it.value.keySet().contains('expression_recount3')) {
            tuple(
                it.key,
    ...
    // DOWNLOAD MUTATIONS
    dChMu = dataCh.map { it ->
        if (it.value.keySet().contains('mutation_tcgabiolinks')) {
            tuple(
                it.key,
    ...
    dChCNV = dataCh
        .map { it ->
            if (it.value.keySet().contains('cnv_tcgabiolinks')) {
                tuple(
                    it.key,
    ...
    dChMe = dataCh.map { it ->
        if (it.value.keySet().contains('methylation_gdc')) {
            tuple(
                it.key,
    ...
    // DOWNLOAD CLINICAL
    dChCl = dataCh.map { it ->
        if (it.value.keySet().contains('clinical_tcgabiolinks')) {
            tuple(
                it.key,
...
def get_expression_recount3(x) {
    if (x.value.get('expression_recount3')) {
        println('expression_recount3 exists')

        def filePath = params.inputFile ?: null
        // Assuming you have a parameter named inputFile
...
def get_mutation_tcgabiolinks(x) {
    if (x.value.get('mutation_tcgabiolinks')) {
        println('mutation_tcgabiolinks exists')
        a = tuple(
            x.key,
...
def get_cnv_tcgabiolinks(x) {
    if (x.value.get('cnv_tcgabiolinks')) {
        println('cnv_tcgabiolinks exists')
        a = tuple(
            x.key,
...
def get_methylation_gdc(x) {
    if (x.value.get('methylation_gdc')) {
        println('methylation_gdc exists')
...
def get_clinical_tcgabiolinks(x) {
    if (x.value.get('clinical_tcgabiolinks')) {
        println('clinical_tcgabiolinks exists')
        a = tuple(
            x.key,
